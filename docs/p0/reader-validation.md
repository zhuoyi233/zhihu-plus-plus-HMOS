...
2026-08-14（API 26 虚拟机，登录态）：知乎 `equation` 端点对无 Cookie 或纯游客请求返回 302（unhuman 安全验证页），导致公式 SVG 下载失败。修复后公式请求携带应用会话 Cookie（含 `z_c0`）与桌面 UA，返回 `200 image/svg+xml`；登录态下滚动长文，74 个公式全部原生解码（`API 24 原生公式已解码 74/74`），无降级、无崩溃。游客态在风控 IP 下仍可能被知乎安全验证拦截，属于外部网络条件。
