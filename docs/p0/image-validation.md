...
| JPEG 网络图成功回调 | 通过 |
| GIF 成功回调 | 通过 |
| GIF 两帧像素变化 | `96 / 5445` 个采样点变化 |
| 无效地址失败提示 | 通过 |
| 点击重试后恢复 | 通过 |
| 原生全屏预览打开/关闭 | 通过 |

GIF 像素差只比较组件可见区域，排除了状态栏时间变化；API 24 观察到非零变化，因此不是仅加载首帧。

## 自动化与构建

- API 24 SDK 下 Debug HAP：`BUILD SUCCESSFUL`。
- 2 个解析测试覆盖知乎懒加载图片优先级、图片说明、协议相对地址归一化和危险来源拒绝，并已纳入最终 46 个 Hypium 用例。
...
- 公式图片由 `P0-MATH-01` 独立验证，普通 JPEG/GIF 结论不与公式路线混用。

`P0-IMG-01` 的网络图、失败重试、GIF 和预览策略已经明确，并在 API 24 上通过，因此本项标记为通过。
