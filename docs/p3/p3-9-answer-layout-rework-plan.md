...
**状态：✅ 已完成（2026-08-14，commit `2c3c28fa`）**
...
1. `ContentDetailPages.ets` 新增 `bottomActionBar` @Builder：
   - **左组胶囊**：赞同（图标+数字）/ 反对（图标），沿用 P3-3 `ContentVoteController`
     状态（`voteState/voteupCount`），容器按状态变色（Neutral 浅蓝底 `brand_primary_container`、
     激活实蓝底 `brand_primary`）。
...
**设备实测**（ZhihuPlus_API26 + 登录态）：底部栏渲染（赞同 631/反对/收藏/评论 74）、
底部评论按钮打开评论弹层、投票乐观更新往返（反对↔已反对）、收藏弹窗打开，全部通过。
同时移除正文上方旧的收藏/评论按钮（避免双入口），屏蔽作者/屏蔽关键词保留正文卡片。

### 阶段 B：顶部折叠作者栏

**状态：✅ 已完成（2026-08-14，commit `3291c4f3`）**
...
**实现**：自定义 `detailTopBar` @Builder（Stack 叠加于正文之上，返回键由 NavDestination
系统提供不重复）：未滚动显示作者行（头像占位圆 + 作者名 + headline），滚动后收起为标题
单行；`NativeContentDocument` 新增 `onScrollOffset` 回调（List `onScrollIndex` 驱动）。
顶栏 `zIndex(2)` + `hitTestBehavior(Default)` 确保点击不被正文 Column 拦截。

**设备实测**（ZhihuPlus_API26 + 登录态）：展开态作者行渲染（头像圆/作者名）、滚动折叠
为标题行、作者行点击跳用户主页（`P1DestinationName.PEOPLE`）均通过；返回后滚动位置
保留（P3-7 回归）。注意：顶栏展示纯视觉，不触发数据重载。

### 阶段 C：底部菜单弹窗

**状态：✅ 已完成（2026-08-14，commit `b1d74dfb`）**
...
等价实现）；「朗读/导出」为禁用占位（依赖系统能力）。底部栏补 `zIndex(2)` 修复
点击被正文 Column 拦截。
...
1. 社交信用行：从统计文本拆分「N 人赞同了该回答」（可点 → 赞同者列表，
   ...
   服务未接，仅模型预留）。——统计文本保留现状，赞同者列表/AIGC 人数模型预留。
2. 认可徽章流：`endorsements` 模型 + FlowRow 胶囊（按 token 表映射颜色），
   上游字段接入 `ContentDetailDecoder`。——已实现：`Answer/Article.endorsements`
   解码（安全 token 组 `GBK/GBL/GRD/GYL` 校验 + alpha 钳制），Flex 换行胶囊渲染，
   未知 token 回退主题色。
3. IP 属地：正文底部右对齐灰色小字（数据来自详情接口，若未返回则隐藏）。——
   已实现：`Answer/Article.ipInfo` 解码，正文头部显示，无值隐藏。
4. 阅读进度条：右侧竖条，随正文滚动更新（ArkUI 用 `Scroll` 偏移比）。——
   已实现：`NativeContentDocument.onScrollPercent`（按 List `onScrollIndex`/
   总块数比例），右侧 3px 竖条随滚动伸缩。
5. 底部栏自动隐藏：滚动下移 + 上滑恢复（`Scroll` 方向判定 + 动画）。——未实现
   （滚动隐藏与顶栏折叠/进度条共用滚动回调，需额外方向判定；列入后续）。

**设备实测**（ZhihuPlus_API26 + 登录态）：菜单弹出（更多操作/复制链接/分享/朗读占位/
导出占位）、复制链接触发关闭菜单、阅读进度条滚动更新，均通过。认可徽章/IP 属地依赖
接口返回（当前账号大部分回答无 endorsements），解码正确性由单测覆盖。
...
- [x] 回答详情页底部操作栏：赞同（图标+数字）/反对胶囊组，状态切换变色；
- [x] 收藏 IconButton（已收藏橙色）、评论按钮（图标+数字）位置与上游一致；
- [x] …更多打开底部菜单弹窗（分享/复制链接等入口可达）；
- [x] 顶部两行折叠作者栏：收起单行标题、展开头像+作者名+简介，滚动联动；
- [x] 作者行点击进用户主页，返回后正文滚动位置保留（P3-7 回归）；
- [ ] 「N 人赞同了该回答」社交信用行可点开赞同者列表（如有数据）；
- [x] 认可徽章 FlowRow 胶囊流渲染正确（token 色板映射）；
- [x] IP 属地右对齐显示（接口有值时）；
- [x] 阅读进度条随滚动更新；
- [ ] 底部栏滚动自动隐藏、上滑恢复；
- [x] 移除正文上方互动区后无残留按钮、无布局跳变；
- [x] 原有屏蔽作者/屏蔽关键词/收藏/评论入口行为回归。
