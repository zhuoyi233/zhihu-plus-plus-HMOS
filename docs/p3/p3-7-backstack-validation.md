...
- [x] 首页/热榜/关注/日报列表下滑后进入回答/文章/问题详情，返回后列表停留在原位置（含分页加载后）；
...
- [x] 同一用户主页在栈中出现过的场景（详情→作者→返回→再点作者）不再产生重复页面；
...
1. **列表滚动位置恢复**：首页推荐流下滑到「技术没有变差。1993年侏罗纪公园…」位置 →
   进入回答详情 → Back 返回 → 首页仍停留在同一位置（重显幂等守卫生效，未重载重置）；
2. **导航去重**：设置页快速连点两次「我的收藏夹」→ 只入栈一次，一次 Back 直接回设置页，
   无重复收藏夹页（`findP1DestinationStackIndex` 栈级身份去重生效）。
