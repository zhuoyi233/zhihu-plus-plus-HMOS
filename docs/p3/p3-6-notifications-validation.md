...
- [ ] 设置页「通知」进入通知页，未登录显示「去登录」，登录后自动重载；
- [ ] 分类 Tab 切换（消息/关注订阅/赞同感谢），未读数角标正确；
- [ ] 通知列表分页加载无重复，点击通知单条已读并跳转目标（问题/回答/文章/用户/想法）；
- [ ] 「全部已读」乐观置灰未读、失败回滚并展示错误；
- [ ] 通知设置页四类开关读写持久化，重启后保持；邀请回答默认关闭；
