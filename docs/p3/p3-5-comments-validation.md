...
- [x] 回答/文章详情页「评论」按钮打开评论弹层，未登录显示「去登录」；
- [x] 根评论列表按最热/最新排序切换、分页无重复，作者名可进用户主页并返回后弹层仍在原位置；
- [x] 展开/收起内联子评论，「查看全部 N 条回复」进入子评论列表并可返回；
...
1. **入口**：回答详情页「评论」按钮（`p3_answer_detail_comment`）打开评论弹层；
2. **根评论列表**：真实评论加载（作者、内容、回复/点赞按钮、查看全部回复折叠）；
3. **排序切换**：最热/最新切换后作者列表变化（时间序生效）；
4. **子评论**：展开「查看全部 9 条回复」→ 子评论列表（含置顶标记、返回按钮）；
5. **修复**：评论作者 `id` 为知乎 people 风格 32 位十六进制串（如
   `7984d572517b489e42f418a63eae0b09`），原解码器仅接受纯数字导致整页解码失败；
   已新增 `authorId` 宽松校验并补测试用例。

> 说明：以上写接口（发表/回复/删除/点赞）与真实登录态依赖设备实测。
> 本轮已实测编译 + Hypium 自动化：`verify-harmony.ps1 -SkipDependencyInstall -SkipBuild`
> 通过，**386/386 用例全绿**（基线 327 + 本切片新增 27 + 修复后新增 hex 用例）。
